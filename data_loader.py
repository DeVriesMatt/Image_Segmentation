...
class ImageFolder(data.Dataset):
	def __init__(self, root, image_size=48, mode='train', augmentation_prob=0.4):  # TODO: change image size for patch
		...
		self.image_paths = list(map(lambda x: os.path.join(root, x), os.listdir(
			root)))  # root[:-1]+'_GT/'  # TODO: test on only GT images as input list(map(lambda x: os.path.join(root, x), os.listdir(root)))
	...
	def __getitem__(self, index):
		...
		filename = image_path[
				   -9:]  # TODO: Change when using image patches because of naming convention: 17 for patch 9 for not patch
